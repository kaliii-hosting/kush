...
const MerchProductSlider = ({ onCartClick }) => {
  ...
  // Filter products from "my store" category
  const merchProducts = shopifyProducts.filter(product => {
    // Check product type
    if (product.productType?.toLowerCase() === 'my store' || 
    ...
    if (product.collections?.some(collection => 
      collection.title?.toLowerCase() === 'my store' ||
      collection.handle?.toLowerCase() === 'my-store' ||
    ...
    // Check vendor
    if (product.vendor?.toLowerCase() === 'my store' ||
